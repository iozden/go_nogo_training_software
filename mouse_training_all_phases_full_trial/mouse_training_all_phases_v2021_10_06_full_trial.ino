...
{ 
  switch (trainingPhase)
  {
      ...
      case 4:
        ...
        detect.start(odorPortExit,settleTime);   
        if (eventFlag==odorPortExit){analogWrite(DAC0,0);analogWrite(DAC1,0);laser.stop2();endTrial(earlyResp,punishment);break;}
  
        set_pins(finalValve,air,odor); 
        detect.start(odorPortExit,odorSettleTime);   
        if (eventFlag==odorPortExit){clear_pins(odor,air,finalValve);analogWrite(DAC0,0);analogWrite(DAC1,0); laser.stop2(); endTrial(earlyResp,punishment);break;}
