  ...
  switch (trainingPhase)
  {
      ...
      case 4:
        ...
        if (eventFlag==0) 
        {
          if (Go==0)
          {
            if (correctStim==1) {laser.start();analogWrite(DAC1,laserSynch);}  
          ...
          {
            if (incorrectStim==1) {laser.start();analogWrite(DAC1,laserSynch);} 
            analogWrite(DAC1,feedbackBadSynch);deliverSound(incorrectSound,feedbackDuration); analogWrite(DAC0,0); analogWrite(DAC1,0);endTrial(incorrectResp,punishment+iti); 
            break;
          }   
        }
