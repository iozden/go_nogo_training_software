...
class BehaviorGUI:
    def __init__(self, master):
        ...
        self.training_phase.insert(10,'4')
        ...
        self.maxRespTime.insert(10,'1500')
        ...
        self.rewardDuration.insert(10,'100')
        ...
        self.odor1_flag = IntVar(value=0)
        self.odor1 = Checkbutton(master,text="Odor 1 learned",variable=self.odor1_flag).place(relx=0.75,rely=0.78)
        
        self.odor2_flag = IntVar(value=0)
        self.odor2 = Checkbutton(master,text="Odor 2 learned",variable=self.odor2_flag).place(relx=0.75,rely=0.82)

        self.odor3_flag = IntVar(value=0)
        self.odor3 = Checkbutton(master,text="Odor 3 learned",variable=self.odor3_flag).place(relx=0.75,rely=0.86)

        self.odor4_flag = IntVar(value=0)
        self.odor4 = Checkbutton(master,text="Odor 4 learned",variable=self.odor4_flag).place(relx=0.75,rely=0.90)        
        ...
        if (len(a)>1) and (a[0]=='A'):
            ...
            if len(a)>1:
                ...
                if nTrials>0:
                        ...
                        learned1=self.odor1_flag.get();
                        learned2=self.odor2_flag.get();
                        learned3=self.odor3_flag.get();
                        learned4=self.odor4_flag.get();
                        ...
                        indsOdor1=[i for i, z in enumerate(trial_types) if z == 1]
                        indsOdor2=[i for i, z in enumerate(trial_types) if z == 2]
                        indsOdor3=[i for i, z in enumerate(trial_types) if z == 3]
                        indsOdor4=[i for i, z in enumerate(trial_types) if z == 4] 
                        
                        nOdor1=len(indsOdor1);
                        correctOdor1=[correct_trials[i] for i in indsOdor1];      
                        nOdor2=len(indsOdor2);
                        correctOdor2=[correct_trials[i] for i in indsOdor2];
                        nOdor3=len(indsOdor3);
                        correctOdor3=[correct_trials[i] for i in indsOdor3];
                        nOdor4=len(indsOdor4);
                        correctOdor4=[correct_trials[i] for i in indsOdor4];
                        
                        if not learned1:
                            if np.sum(correctOdor1[-3:]) > 2:
                                self.odor1_flag.set(1);
                                
                        if not learned2:
                            if np.sum(correctOdor2[-3:]) > 2:
                                self.odor2_flag.set(1);    
                                
                        if not learned3:
                            if np.sum(correctOdor3[-3:]) > 2:
                                self.odor3_flag.set(1);                                

                        if not learned4:
                            if np.sum(correctOdor4[-3:]) > 2:
                                self.odor4_flag.set(1);                                
                        ...
                        if p1:
                            if indsOdor1:
                                y1=np.cumsum(correctOdor1)
                                y2=range(1,len(indsOdor1)+1)
                                self.ax.plot(y2,y1/y2,'k')
                                if len(y2)>9:                            
                                    ys=np.convolve(correctOdor1,self.kern,'valid')/10
                                    ys2=range(1,len(ys)+1)
                                    self.ax2.plot(ys2,ys,'k')                                     

                        if p2:
                            if indsOdor2:
                                y1=np.cumsum(correctOdor2)
                                y2=range(1,len(indsOdor2)+1)
                                self.ax.plot(y2,y1/y2,'r')                          
                                if len(y2)>9:
                                    ys=np.convolve(correctOdor2,self.kern,'valid')/10
                                    ys2=range(1,len(ys)+1)
                                    self.ax2.plot(ys2,ys,'r')                          

                        if p3:
                            if indsOdor3:
                                y1=np.cumsum(correctOdor3)
                                y2=range(1,len(indsOdor3)+1)
                                self.ax.plot(y2,y1/y2,'c')
                                if len(y2)>9:
                                    ys=np.convolve(correctOdor3,self.kern,'valid')/10
                                    ys2=range(1,len(ys)+1)
                                    self.ax2.plot(ys2,ys,'c')                                      
                                    
                        if p4:
                            if indsOdor4:
                                y1=np.cumsum(correctOdor4)
                                y2=range(1,len(indsOdor4)+1)
                                self.ax.plot(y2,y1/y2,'m')                           
                                if len(y2)>9:
                                    ys=np.convolve(correctOdor4,self.kern,'valid')/10
                                    ys2=range(1,len(ys)+1)
